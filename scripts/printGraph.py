...
def main():
    print("Small skript to visualize small graphs")
    filename = input("Enter the path to the .gl or .fmi file: ").strip()
    if(filename.endswith(".gl")):
        nodes, edges = read_gl_file(filename)
    elif(filename.endswith(".fmi")):
        #../data/triangulation/polyanyaWorstCase.fmi
        nodes, edges = read_fmi_file(filename)
    else:
        print("illegal file ending")
